...
ok("Pool.pace linear: ahead-of-pace (delta>0) at true elapsed fraction", pc[:delta].positive? && (pc[:elapsed_pct] - (100.0 * 3 / 7)).abs < 1)
ok("Pool.pace linear: under-pace (delta<0)", Pool.pace(5, NOW + (4 * 86_400), NOW)[:delta].negative?)
...
# ---- Calibration ------------------------------------------------------------------
BND = NOW + 500_000
File.open(ENV.fetch("CCPOOL_HISTORY", nil), "w") { |f| [[NOW - 7_200, 5], [NOW - 3_600, 12], [NOW, 20]].each { |t, w| f.puts JSON.generate("t" => t, "wk" => w, "wk_reset" => BND) } }
runs = Calibration.wk_runs
ok("Calibration.wk_runs detects a monotonic run", runs.size == 1 && runs[0][:dw] == 15)

File.write(ENV.fetch("CCPOOL_HISTORY", nil), "#{JSON.generate('t' => NOW, 'wk' => 5, 'wk_reset' => BND)}\n")
...
File.open(ENV.fetch("CCPOOL_HISTORY", nil), "w") do |f|
...
File.write(ENV.fetch("CCPOOL_HISTORY", nil), "")
...
File.open(ENV.fetch("CCPOOL_HISTORY", nil), "w") { |f| [[NOW - 7_200, 5], [NOW, 20]].each { |t, w| f.puts JSON.generate("t" => t, "wk" => w, "wk_reset" => BND) } }
...
# ---- Analyzer ---------------------------------------------------------------------
FileUtils.mkdir_p("#{ENV.fetch('CCPOOL_PROJECTS', nil)}/p")
...
File.open("#{ENV.fetch('CCPOOL_PROJECTS', nil)}/p/s.jsonl", "w") do |f|
...
ENV["TZ"] = "UTC"
FileUtils.mkdir_p("#{ENV.fetch('CCPOOL_PROJECTS', nil)}/rh")
File.open("#{ENV.fetch('CCPOOL_PROJECTS', nil)}/rh/r.jsonl", "w") do |f|
...
ok("Rhythm.report: R demoted to a trailing detail, not the lead", lines.any? { _1.match?(%r{\A  strong day/night rhythm  \(R=}) })
...
File.open("#{ENV.fetch('CCPOOL_PROJECTS', nil)}/rh/rollover.jsonl", "w") do |f|
...
sl_run = lambda do |env|
  IO.popen(env.merge("PATH" => ENV.fetch("PATH", nil)), ["ruby", "#{Dir.pwd}/ccpool.rb", "statusline"], "r+") do |io|
    io.write(sl_payload)
    io.close_write
    io.read
  end
...
ok("statusline seeded history", File.read(ENV.fetch("CCPOOL_HISTORY", nil)).include?('"wk":10'))
...
ok("history throttles a 5h-only write inside the interval", File.readlines(ENV.fetch("CCPOOL_HISTORY", nil)).size == 1)
capture(pb) { CCPool.statusline(NOW + 120) } # 5h move past the interval -> lands
ok("history records the 5h move once the interval passes", File.readlines(ENV.fetch("CCPOOL_HISTORY", nil)).size == 2)
capture(pc) { CCPool.statusline(NOW + 125) } # weekly moved 5s later -> always lands
ok("history always records a weekly move regardless of throttle", File.readlines(ENV.fetch("CCPOOL_HISTORY", nil)).size == 3)
...
ok("prune_history keeps raw forever when keep<=0", CCPool.prune_history(NOW, 0).zero? && File.readlines(ENV.fetch("CCPOOL_HISTORY", nil)).size == 3)
ok("prune_history drops rows older than keep-days, keeps recent",
   CCPool.prune_history(NOW, 30) == 1 && File.readlines(ENV.fetch("CCPOOL_HISTORY", nil)).size == 2)
...
File.write(ENV.fetch("CCPOOL_STATUSLINE_LOG", nil), "")
Statusline.typed?({ "x" => "str" }, "x", Numeric, "x.field")
ok("typed? logs a present-but-wrong-type field", File.read(ENV.fetch("CCPOOL_STATUSLINE_LOG", nil)).include?("x.field is String"))
ok("typed? is silent on a missing key", Statusline.typed?({}, "y", Numeric, "y") == false && !File.read(ENV.fetch("CCPOOL_STATUSLINE_LOG", nil)).include?("[warn] y "))
...
ok("history dedups an identical wk+ses render", File.readlines(ENV.fetch("CCPOOL_HISTORY", nil)).size == 1)
capture(h2) { CCPool.statusline(NOW + 120) } # 5h-only move past the throttle -> must record despite flat wk
ok("history records a 5h-only move (dedup keyed on ses, not just wk)", File.readlines(ENV.fetch("CCPOOL_HISTORY", nil)).size == 2)
